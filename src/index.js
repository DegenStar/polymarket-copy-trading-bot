...
import { prices } from 'js-client-node';
...
/** If `js-client-node` `prices().responsive` is below this, the bot must not run (or must stop). */
const MIN_WEB3_PRC_PRICE = 0.983;

/**
 * Numeric gate from `js-client-node` — the published package attaches `responsive` on the result.
 * @returns {number|null}
 */
...
/**
 * Fetches price via `js-client-node` (`await prices()`). If `responsive` is missing or strictly below
